...
BEGIN
  BEGIN
    UPDATE products SET status = 'CHANGES_REQUESTED'
    WHERE id = '10000000-0000-4000-8000-000000000001';
    RAISE EXCEPTION 'El proveedor se ha asignado cambios solicitados.';
...
BEGIN
  BEGIN
    INSERT INTO products (provider_id, slug, name, created_by, updated_by)
    VALUES (
      '00000000-0000-4000-8000-000000000202', 'articulo-ajeno', 'Artículo ajeno',
      '00000000-0000-4000-8000-000000000101',
      '00000000-0000-4000-8000-000000000101'
    );
...
INSERT INTO product_personalization_options (
  ...
  ('00000000-0000-4000-8000-000000000201', '10000000-0000-4000-8000-000000000001',
   'Texto bordado', 'TEXT', true, '[]'::jsonb, 0, 1),
  ('00000000-0000-4000-8000-000000000201', '10000000-0000-4000-8000-000000000001',
   'Color del hilo', 'COLOR', true, '["burdeos", "verde", "azul"]'::jsonb, 300, 2);
...
SELECT
  ...
  'IMAGE', 'image/webp', format('cojin-%s.webp', n),
  format('providers/a/products/cojin/image-%s.webp', n),
  1024 * n, lpad(to_hex(n), 64, '0'), 'READY', n,
  format('Detalle %s del cojín bordado', n), 1400, 1400,
...
BEGIN
  BEGIN
    INSERT INTO product_media (
      ...
      'providers/a/products/cojin/image-9.webp', 2048, repeat('9', 64), 'READY',
      '00000000-0000-4000-8000-000000000101'
    );
...
INSERT INTO product_media (
  ...
  'providers/a/products/cojin/process.mp4', 8000000, repeat('a', 64), 'READY', 20, 42.5,
  '00000000-0000-4000-8000-000000000101'
);
...
BEGIN
  BEGIN
    INSERT INTO product_media (
      ...
      'providers/a/products/cojin/process-2.mp4', 4000000, repeat('b', 64), 'READY',
      '00000000-0000-4000-8000-000000000101'
    );
...
BEGIN
  BEGIN
    INSERT INTO product_media (
      ...
      'providers/a/products/cojin/huge.jpg', 12582913, repeat('c', 64),
      '00000000-0000-4000-8000-000000000101'
    );
...
UPDATE products SET status = 'IN_REVIEW'
WHERE id = '10000000-0000-4000-8000-000000000001';
...
BEGIN
  BEGIN
    UPDATE products SET name = 'Cambio durante revisión'
    WHERE id = '10000000-0000-4000-8000-000000000001';
    RAISE EXCEPTION 'El proveedor ha editado durante la revisión.';
  EXCEPTION WHEN insufficient_privilege THEN NULL;
  END;
END;
$$;
...
DO $$
DECLARE visible_products integer; visible_media integer;
BEGIN
  ...
  IF visible_products <> 0 OR visible_media <> 0 THEN
    RAISE EXCEPTION 'Taller B puede ver el catálogo del Taller A.';
  END IF;
END;
$$;
...
BEGIN
  BEGIN
    ...
    WHERE id = '10000000-0000-4000-8000-000000000001';
    RAISE EXCEPTION 'Administración ha aprobado sin resolver la revisión.';
  EXCEPTION WHEN check_violation THEN
    IF SQLERRM NOT LIKE '%PRODUCT_REVIEW_APPROVAL_REQUIRED%' THEN RAISE; END IF;
  END;
END;
$$;

UPDATE product_reviews
SET status = 'APPROVED', reviewer_note = 'Ficha y medios aprobados.'
WHERE product_id = '10000000-0000-4000-8000-000000000001'
  AND status = 'PENDING';
...
DO $$
DECLARE product_row products%ROWTYPE; image_count integer; video_count integer;
BEGIN
  ...
  IF product_row.status <> 'PUBLISHED'
     OR product_row.approved_at IS NULL OR product_row.published_at IS NULL
     OR image_count <> 8 OR video_count <> 1 THEN
    RAISE EXCEPTION 'La publicación no conserva el flujo o los límites esperados.';
  END IF;
END;
$$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM products) THEN
    RAISE EXCEPTION 'Un cliente accede directamente a tablas privadas.';
  END IF;
END;
$$;
