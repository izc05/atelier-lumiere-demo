...
assert.ok(bootstrap.includes("navigationCheckpoint = 'u3.17n.3'"));

for (const token of [
  'safeBaseDraw',
  'frame = function u317n3SafeFrame',
  ...
  'RENDER LIVE',
  "rendererWatchdog = 'u3.17n.3'",
  "navigationCheckpoint = 'u3.17n.3'"
]) {
  assert.ok(watchdog.includes(token), `U3.17N.3 debe conservar ${token}`);
}
...
console.log('U3.17N.3 renderer watchdog contract: OK');