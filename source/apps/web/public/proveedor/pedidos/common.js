(() => {
  ...
  const ORDER_CLASSES = Object.freeze({
  ...
  function statusBadge(status, type = "order") {
    const labels = type === "request" ? REQUEST_LABELS : ORDER_LABELS;
    const classes = type === "request" ? REQUEST_CLASSES : ORDER_CLASSES;
    return element("span", `status ${classes[status] ?? "pending"}`, labels[status] ?? status);
  }
  ...
  function wireLogout(id = "logout-button") {
  ...
  function keyValue(label, value) {
    const row = element("div", "key-value");
    row.append(element("span", "", label), element("strong", "", value || "No indicado"));
    return row;
  }
  function address(value) {
    if (!value || typeof value !== "object") return "No indicada";
    return [
      value.line1,
      value.line2,
  ...
  window.AtelierOrders = Object.freeze({
    ORDER_LABELS,
    REQUEST_LABELS,
    byId,
    element,
