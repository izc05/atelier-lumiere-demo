(() => {
  ...
  function slotCard(slot) {
    ...
    if (slot.media) {
      ...
      empty.append(node("strong", "", "Bordado cinematográfico activo"), node("span", "", "Sube una fotografía para sustituir el fondo únicamente en este formato."));
  ...
  async function inject() {
    if (injecting || target.hidden) return;
    injecting = true;
    try { await refreshEntrySection(); } catch { /* el editor principal mostrará el error de sesión */ }
