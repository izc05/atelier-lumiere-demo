...
const ROUTES = new Map([
  ["/internal/payment-sandbox/begin", "/api/payment-sandbox/begin"],
  ["/internal/payment-sandbox/simulate", "/api/payment-sandbox/simulate"]
]);
...
async function readBody(request) {
  ...
  for await (const chunk of request) {
    ...
    if (size > MAX_BODY_BYTES) {
      const error = new Error("La petición supera 16 KB.");
...
export function createPaymentSandboxWebHandler({
  baseHandler,
  ...
  if (typeof baseHandler !== "function") {
    throw new TypeError("createPaymentSandboxWebHandler necesita un handler base.");
  }
  const apiBase = new URL(apiInternalUrl);

  return async function paymentSandboxWebHandler(request, response) {
    const url = new URL(request.url ?? "/", "http://localhost");
    const upstreamPath = ROUTES.get(url.pathname);
    if (!upstreamPath) return baseHandler(request, response);

    if (!enabled) {
      response.writeHead(404, secureHeaders({ "Content-Type": "text/plain; charset=utf-8" }));
    ...
    if (crossSite(request)) {
      sendJson(response, 403, {
        ...
        message: "La simulación debe iniciarse desde Atelier Lumière."
      });
      return;
    }
    ...
    try {
      ...
      logger.error("No se pudo completar el pago sandbox web.", {
        code: typeof error?.code === "string" ? error.code : "PAYMENT_SANDBOX_PROXY_FAILED"
      });
      sendJson(response, error?.statusCode ?? 502, {
        error: error?.statusCode ? "INVALID_REQUEST" : "API_UNAVAILABLE",
        message: error?.message ?? "El simulador de pago no responde."
      });
    }
  };
}
