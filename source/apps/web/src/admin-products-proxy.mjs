...
function isAdminProductsPage(pathname) {
  return pathname === "/admin/articulos"
    || pathname === "/admin/articulos/"
    || pathname === "/admin/articulos/revisar"
    || pathname === "/admin/articulos/revisar/";
}
...
export function createAdminProductsWebHandler({
  baseHandler,
  ...
  if (typeof baseHandler !== "function") {
    throw new TypeError("createAdminProductsWebHandler necesita un handler base.");
  }
  if (enableAdminUi && (typeof apiAdminToken !== "string" || apiAdminToken.length < 32)) {
    throw new TypeError("DEV_ADMIN_TOKEN debe estar configurado para revisar artículos.");
  }
  const apiBase = new URL(apiInternalUrl);

  return async function adminProductsWebHandler(request, response) {
    const url = new URL(request.url ?? "/", "http://localhost");
    const match = url.pathname.match(ADMIN_PRODUCTS_PATTERN);
    ...
    if (match) {
      ...
      try {
        ...
        logger.error("No se pudo completar el proxy de revisión administrativa.", {
          code: typeof error?.code === "string" ? error.code : "ADMIN_PRODUCTS_PROXY_FAILED"
        });
        if (!response.headersSent) {
          sendJson(response, 502, {
            error: "API_UNAVAILABLE",
            message: "La revisión de artículos no responde."
          });
    ...
    if (["GET", "HEAD"].includes(request.method ?? "GET") && isAdminProductsPage(url.pathname)) {
      if (!enableAdminUi) {
        sendNotFound(response);
        return;
      }
