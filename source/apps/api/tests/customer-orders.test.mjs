...
function address() {
  return { line1: "Calle cliente 8", city: "Granada", postalCode: "18001", country: "ES" };
}
...
test("el enlace de un solo uso crea una sesión y permite al cliente gestionar sus pedidos", { skip: !connectionString }, async (t) => {
  ...
  await database.withContext(ADMIN, async (tx) => {
    await tx.query(
      `INSERT INTO users(id,email,display_name,status,email_verified_at,two_factor_enabled)
       VALUES($1,$2,'Cliente privado','ACTIVE',now(),false)`,
      [customerId, email]
    );

    const checkoutSql = `INSERT INTO checkout_batches(
      id,customer_user_id,checkout_reference,currency,customer_name,
      contact_email,shipping_address,status,submitted_at
    ) VALUES($1,$2,$3,'EUR','Cliente privado',$4,$5::jsonb,'SUBMITTED',now())`;
    await tx.query(checkoutSql, [
      checkoutAId,
      customerId,
    ...
    const orderSql = `INSERT INTO provider_orders(
    ...
    ) VALUES($1,$2,$3,$4,$5,'PENDING_CONFIRMATION','EUR',$6,$7,$8,$9,$10,
      'Cliente privado',$11,$12::jsonb)`;
    await tx.query(orderSql, [
      orderAId,
      checkoutAId,
      PROVIDER_A,
      customerId,
      `AL-CUSTOMER-A-${suffix}`,
      4800,
      500,
      5300,
      3,
    ...
    await tx.query(
      `INSERT INTO order_items(
        id,order_id,provider_id,customer_user_id,item_type,product_name,
        quantity,unit_price_cents,line_total_cents,currency,personalization
      ) VALUES($1,$2,$3,$4,'CUSTOM','Bordado privado',1,4800,4800,'EUR',$5::jsonb)`,
      [itemId, orderAId, PROVIDER_A, customerId, JSON.stringify({ name: "Adriana" })]
    );
    await tx.query(
      `INSERT INTO custom_requests(
        id,order_id,order_item_id,provider_id,customer_user_id,title,brief,
        status,quoted_price_cents,currency
      ) VALUES($1,$2,$3,$4,$5,'Bordado con nombre',
        'El taller ha preparado un presupuesto que debe aprobar únicamente el cliente.',
        'QUOTED',4800,'EUR')`,
      [requestId, orderAId, itemId, PROVIDER_A, customerId]
    );
  });
  ...
  const issued = await auth.issueAccess({ customerUserId: customerId, checkoutId: checkoutAId });
  assert.match(issued.accessToken, /^[A-Za-z0-9_-]{32,180}$/);

  await database.withContext(ADMIN, async (tx) => {
    const result = await tx.query(
      "SELECT token_hash FROM customer_order_access_tokens WHERE id=$1",
      [issued.accessId]
    );
  ...
  await assert.rejects(
    () => auth.consumeAccess(issued.accessToken),
  ...
  assert.equal(new Set(orders.map((order) => order.checkoutId)).size, 2);

  const detail = await service.get(session.context, orderAId);
  assert.equal(detail.items.length, 1);
  assert.equal(detail.customRequests[0].status, "QUOTED");

  const message = await service.addCustomMessage(session.context, requestId, {
    body: "Confirmo el color y la escritura indicados en la solicitud."
  });
  assert.equal(message.authorRole, "CUSTOMER");

  const approved = await service.approveQuote(session.context, requestId, {
    expectedVersion: detail.customRequests[0].version
  });
  assert.equal(approved.status, "APPROVED");
  assert.equal(approved.quotedPriceCents, 4800);

  const cancelled = await service.cancelOrder(session.context, orderBId, {
    expectedVersion: orders.find((order) => order.id === orderBId).version
  });
  assert.equal(cancelled.status, "CANCELLED");

  await assert.rejects(
    () => service.cancelOrder(session.context, orderAId, {
      expectedVersion: orders.find((order) => order.id === orderAId).version + 1
    }),
    (error) => ["ORDER_VERSION_CONFLICT", "ORDER_CANNOT_BE_CANCELLED"].includes(error?.code)
  );
  ...
  await database.withContext(ADMIN, async (tx) => {
    ...
    const sessions = await tx.query(
      "SELECT token_hash,user_agent_hash FROM customer_sessions WHERE id=$1",
      [consumed.session.id]
    );
