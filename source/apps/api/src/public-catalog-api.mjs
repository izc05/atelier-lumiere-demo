...
function handleError(response, error, logger) {
  ...
  sendJson(response, 500, { error: "INTERNAL_ERROR", message: "No se ha podido abrir el catálogo." }, "no-store");
}

function contentDisposition(filename) {
  const encoded = encodeURIComponent(filename).replaceAll("'", "%27");
  const fallback = filename.normalize("NFKD").replace(/[^A-Za-z0-9._ -]/g, "_").slice(0, 120) || "archivo";
...
async function sendMedia(response, opened) {
...
export function createPublicCatalogApiHandler({
  ...
  return async function publicCatalogApiHandler(request, response) {
    ...
    try {
      if (!publicCatalogService) throw new ServiceError("SERVICE_UNAVAILABLE", "La tienda todavía no está disponible.", 503);
      ...
      if (mediaMatch) {
        const width = mediaMatch[3] === "preview" ? previewWidth(url.searchParams.get("width")) : null;
        const rangeRequest = width === null ? request.headers.range : { range: request.headers.range, width };
        await sendMedia(response, await publicCatalogService.openMedia(
          mediaMatch[1], mediaMatch[2], mediaMatch[3], rangeRequest
        ));
        return;
      }

      if (providerMediaMatch) {
        const width = previewWidth(url.searchParams.get("width"));
        const rangeRequest = width === null ? request.headers.range : { range: request.headers.range, width };
        await sendMedia(response, await publicCatalogService.openProviderMedia(
          providerMediaMatch[1], providerMediaMatch[2], rangeRequest
        ));
      }
    } catch (error) {
