...
const COLLECTION_PATH = "/api/admin/blog-posts";
const DETAIL_PATTERN = /^\/api\/admin\/blog-posts\/([0-9a-f-]{36})$/i;
const ACTION_PATTERN = /^\/api\/admin\/blog-posts\/([0-9a-f-]{36})\/(review|publish)$/i;
const PREVIEW_PATTERN = /^\/api\/admin\/blog-posts\/([0-9a-f-]{36})\/media\/([0-9a-f-]{36})\/preview$/i;
...
function handleError(response, error, logger) {
  ...
  logger.error("Error no controlado en la revisión del blog.", {
    code: typeof error?.code === "string" ? error.code : "ADMIN_BLOG_API_FAILED"
  });
...
export function createAdminBlogApiHandler({
  baseHandler,
  adminBlogService,
  authenticateRequest,
  logger = console
  ...
  if (typeof baseHandler !== "function") {
    throw new TypeError("createAdminBlogApiHandler necesita un handler base.");
  }
  if (typeof authenticateRequest !== "function") {
    throw new TypeError("createAdminBlogApiHandler necesita autenticación administrativa.");
  }

  return async function adminBlogApiHandler(request, response) {
    ...
    try {
      if (!adminBlogService) {
        throw new ServiceError("SERVICE_UNAVAILABLE", "La revisión del blog no está disponible.", 503);
      }
      ...
      if (url.pathname === COLLECTION_PATH && request.method === "GET") {
        const posts = await adminBlogService.list(context, {
      ...
      if (detailMatch && request.method === "GET") {
        const post = await adminBlogService.get(context, detailMatch[1]);
      ...
      if (actionMatch && request.method === "POST") {
        ...
        const result = action === "review"
          ? await adminBlogService.decide(context, postId, await readJson(request))
          : await adminBlogService.publish(context, postId);
        sendJson(response, 200, result);
        return;
      }

      if (previewMatch && request.method === "GET") {
        const opened = await adminBlogService.openPreview(
          context,
          previewMatch[1],
          previewMatch[2],
